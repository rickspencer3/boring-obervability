...
class HTTPCheck(Check):
    ...
    def run(self):
        ...
        try:
            ...
            lp = f"""checks,url="{self.url}",name={name},method={self.method},user_id={self.user_id},id={self.id} status=404i,elapsed=0i"""
        ...
        lp = f"""checks,url="{self.url}",name={name},method={self.method},user_id={self.user_id},id={self.id} status={check_response.status_code}i,error={error}i,elapsed={check_response.elapsed.microseconds}i"""
        
        self.detect_anomolies_and_record(self, check_response, lp)

    def detect_anomolies_and_record(self, check, check_response, lp):
